...
class Optimizers:
    def __init__(self, networks: Networks, losses: Losses, placeholders: Placeholders, train_videos, tb_threshold):
        ...
        self.optimizer_D_a = tf.cond(tf.less(placeholders.tb_spatial_a, tb_threshold), lambda: optimize_conditionally(losses.loss_D_a, var_list=networks.discriminator_spatial_a.var_list, global_step=placeholders.global_step), lambda: tf.constant(0, dtype=tf.float32))
        self.optimizer_D_b = tf.cond(tf.less(placeholders.tb_spatial_b, tb_threshold), lambda: optimize_conditionally(losses.loss_D_b, var_list=networks.discriminator_spatial_b.var_list), lambda: tf.constant(0, dtype=tf.float32))
        if (train_videos):
