...
from src.utils.tensor_ops import generate_temp_discriminator_input, layer_frames_in_channels
...
class Losses:
    ...
    def define_discriminator_output_video(self, images: Images, networks: Networks, placeholders: Placeholders):
        self.D_real_frame_a = networks.discriminator(layer_frames_in_channels(images.warped_frames_a))[:,0]
        self.D_fake_frame_a = 1-networks.discriminator(layer_frames_in_channels(images.warped_frames_ba))[:,1]
        self.D_real_frame_b = networks.discriminator(layer_frames_in_channels(images.warped_frames_b))[:,2]
        self.D_fake_frame_b = 1-networks.discriminator(layer_frames_in_channels(images.warped_frames_ab))[:,3]
        self.D_history_fake_frame_a = 1-networks.discriminator(
            layer_frames_in_channels(placeholders.history_fake_temp_frames_a))[:,1]
        self.D_history_fake_frame_b = 1-networks.discriminator(
            layer_frames_in_channels(placeholders.history_fake_temp_frames_b))[:,3]

    def define_discriminator_output_images(self, images: Images, networks: Networks, placeholders: Placeholders):
        self.D_real_image_a = networks.discriminator(images.image_a)[:,0]
        self.D_fake_image_a = 1-networks.discriminator(images.image_ba)[:,1]
        self.D_real_image_b = networks.discriminator(images.image_b)[:,2]
        self.D_fake_image_b = 1-networks.discriminator(images.image_ab)[:,3]
        self.D_history_fake_image_a = 1-networks.discriminator(placeholders.history_fake_a)[:,1]
        self.D_history_fake_image_b = 1-networks.discriminator(placeholders.history_fake_b)[:,3]

    def define_discriminator_activations(self, images: Images, networks: Networks):
        self.D_real_activation_a = networks.discriminator(layer_frames_in_channels(images.warped_frames_a),
                                                                    return_layer_activations=True)
        self.D_real_activation_b = networks.discriminator(layer_frames_in_channels(images.warped_frames_b),
                                                                    return_layer_activations=True)

        self.D_fake_activation_a = networks.discriminator(layer_frames_in_channels(images.warped_frames_ba),
                                                                    return_layer_activations=True)
        self.D_fake_activation_b = networks.discriminator(layer_frames_in_channels(images.warped_frames_ab),
                                                                    return_layer_activations=True)
    ...
    def define_losses(self, images: Images, placeholders: Placeholders, training_config: TrainingConfig, train_videos,
        ...
        self.define_generator_loss(train_images)
        ...
        self.define_pingpong_loss(images, placeholders, training_config.pingpong_loss_coefficient, train_videos)
    ...
    def define_generator_loss(self, train_images):
    ...
    def define_pingpong_loss(self, images: Images, placeholders: Placeholders, pingpong_loss_coeff, train_videos):
    ...
    def define_total_generator_loss(self):
        with tf.name_scope("generator_loss"):
            self.loss_G_ab_final = self.loss_G_spat_ab + self.temp_loss_fade_in_weigth * (self.loss_pingpong_ab) + self.loss_cycle + self.identity_fade_out_weight * self.loss_identity + self.loss_code + self.loss_style
            self.loss_G_ba_final = self.loss_G_spat_ba + self.temp_loss_fade_in_weigth * (self.loss_pingpong_ba) + self.loss_cycle + self.identity_fade_out_weight * self.loss_identity + self.loss_code + self.loss_style
