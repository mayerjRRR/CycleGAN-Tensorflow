...
class CycleGan(object):
    ...
    def write_summary(self, fetched, step, summary_writer):
        ...
        self.steps.set_description(
            'Loss: D_a({:.3f}) D_b({:.3f}) G_ab({:.3f}) G_ba({:.3f}) cycle({:.3f}) identity({:.3f}) code({:.3f})'.format(
    ...
    def add_losses(self, fetches):
        fetches['losses'] = [self.losses.loss_D_a, self.losses.loss_D_b, self.losses.loss_G_spat_ab,
                             self.losses.loss_G_spat_ba, self.losses.loss_cycle, self.losses.loss_identity, self.losses.loss_code]
        return fetches
    ...
    def add_discriminator_optimizer(self, fetches, video_training):
        fetches['discriminator_optimizer'] = [self.optimizers.optimizer_D]
        return fetches
