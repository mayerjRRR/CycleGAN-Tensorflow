...
class CycleGan(object):
    ...
    def __init__(self, training_config: TrainingConfig, train_videos=True, train_images=False):
        ...
        self.placeholders = Placeholders(self.config.batch_size, self.image_shape, self.input_shape,
                                         training_config.frame_sequence_length)
        ...
        self.losses = Losses(self.networks, self.placeholders, self.images, self.config, self.train_videos,
                             self.train_images)
        self.optimizers = Optimizers(self.networks, self.losses, self.placeholders, self.train_videos,
                                     self.config.training_balancer_threshold)
        self.training_balancer = TrainingBalancer(self.losses)
        self.tb_summary = TensorBoardSummary(self.images, self.losses, self.placeholders, self.training_balancer,
                                             self.train_videos,
                                             self.train_images)
        self.savers = Savers(self.networks, self.placeholders, self.config.model_directory,
                             self.config.initialization_model)
    ...
    def init_training_parameters(self, sess, learning_rate):
        epoch_length = 500
        num_initial_iter = self.config.training_iterations - self.config.decay_iterations
        num_decay_iter = self.config.decay_iterations
    ...
    def add_losses(self, fetches):
        fetches['losses'] = [self.losses.loss_D_a, self.losses.loss_D_b, self.losses.loss_G_spat_ab,
                             ...
                             self.losses.loss_identity, self.losses.loss_code]
        return fetches
    ...
    def add_balances(self, fetches, video_training):
        if video_training:
            fetches['balancer'] = [self.training_balancer.spatial_a_balance, self.training_balancer.spatial_b_balance,
                                   self.training_balancer.temporal_balance]
        return fetches
